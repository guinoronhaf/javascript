...
class Bola {
    constructor(arrayBolas, palco) {
        ...
        this.px = Math.floor(Math.random() * (larguraPalco - this.tam)) // impedir que alguma parte da bolinha fique pra fora
        ...
        this.desenhar()
        this.controle = setInterval(this.controlar, 10) // verificar se a bolinha bateu na borda
    ...
    remover = () => {
        ...
        bolas = bolas.filter((b) => {
            if (b.id != this.id) { // retorna só as bolinhas que tem o id diferente
                return b
            }
        })
        this.eu.remove()
    ...
    controleBordas = () => {
        if (this.px + this.tam >= larguraPalco) {
            this.dirx = -1 // -1 inverte a direção
        } else if (this.px <= 0) {
            this.dirx = 1
        }
        if (this.py + this.tam >= alturaPalco) {
            this.diry = -1 // -1 inverte a direção
        } else if (this.py <= 0) {
            this.diry = 1
        }
    }
